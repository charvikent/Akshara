	...
	if (experienceList == "") {
		...
		$.each(experienceList,
						function(i, expObj) {
							var tblRow = "<ul>"
								 + "<li class='dept-box'>"
									+ expObj.experianceName
									+ "</li>"
									+ "<li class='eleven-box '>"
									+ '<a href="editExp.htm?experianceId='
									+ expObj.experianceId
									+ '" class="">Edit</a>'
									+ '</li>'
	...
	$(document).ready(function() {
						...
						//AddForm Validations
								 ...
								 $('#submitId').click(function(e){
												...
												if( $('#experianceDesc').val().length == 0){												   
												
											    if($('#experianceDesc').val().length == 0 ) {
												    $('#experianceDesc').css('color','red');
												    $("#experianceDesc").css("border-color","red");
												    $("#experianceDesc").attr("placeholder","Please Enter experiance");
												    $('#experianceDesc').addClass('your-class');
												    }												     
												    return false;												  
												    } 
												    /* var val=$('#experianceDesc').val().charCodeAt(0);
												    var val1=$('#experianceDesc').val();
												    var regex=new RegExp("^[a-zA-Z0-9][a-zA-Z0-9._& ]+$"); */
												    if($('#experianceDesc').val().length <3 )
												    	{
											...
											$('#addForm').attr('action',"<c:url value='/expAdd.htm'/>");
											$("#addForm").submit();											
						...
						$('#updateId').click(function(event) {
										    ...
										    if($('#experianceName').val().length == 0){										   
										    
										    if($('#experianceName').val().length == 0 ) {
										       $('#experianceName').css('color','red');
										       $("#experianceName").css("border-color","red");
										       $("#experianceName").attr("placeholder","Please Enter experience");
										       $('#experianceName').addClass('your-class');
										    }										     
										    return false;										  
										    } 
										    var val=$('#experianceName').val().charCodeAt(0);
										    var val1=$('#experianceName').val();
										    var regex=new RegExp("^[a-zA-Z0-9][a-zA-Z0-9._& ]+$");
										    if(val==32 || val==95||val==38||val==46||$('#experianceName').val().length <3 )
										    	{
											...
											$('#editForm').attr('action',"<c:url value='/expUpdate.htm'/>");
											$("#editForm").submit();
											event.preventDefault();
										});
								});
...
<script type="text/javascript">
	function duplicateChecks() {
		...
		var catname = $('#experianceDesc').val();
		$.ajax({
					type : "POST",
					url : "expDuplicate.htm",
					data : "experianceName=" + catname,
...
<script type="text/javascript">
	function duplicateEditCheck() {
		var expname = $('#experianceName').val();
		var experianceId = $('#experianceId').val();
		$.ajax({
					type : "POST",
					url : "expDuplicate.htm",
					data : "experianceName=" + expname + "&experianceId="
							+ experianceId ,
					success : function(response) {
						if (response != "") {							
			...
			$.ajax({
						...
						success : function(ExpList) {
							...
							if (ExpList == "") {
								...
								$('#legend2').show();
								$('.holder').show();
								$.each(ExpList,function(i, experienceObj) {
													if (experienceObj.sMsg == "Success" && count == 0) {
														count++;
													...
													//alert(i + ": " +experienceObj.categotyDesc+""+experienceObj.experianceId);
													var tblRow = "<ul>"
														+ "<li class='dept-box'>"
															/* + experianceObj.deptName
															+ " "  */
															+ experienceObj.experianceName
															+ "</li>"
															+ "<li class='eleven-box '>"
															+ '<a href="editExp.htm?experianceId='
															+ experienceObj.experianceId
															+ '" class="">Edit</a>'
															+ '</li>'
															+ "<li class='ten-box last'>"
															+ "<a href='javascript:void(0)' id='"
...
function searchFunction() {
	...
	if (searchId == "" || searchId == null) {
		jAlert('please enter experience', 'Alert Box');
		return false;
	}
...
<script>
 function dataClear(){
	 ...
	 $('#experianceDesc').val("");
	  removeBorder('experianceDesc');
	  $("#experianceDesc").attr("placeholder","");
	  document.getElementById("addsus").style.display = "none";
	  document.getElementById("addfail").style.display = "none";
	  document.getElementById("upsus").style.display = "none";
	  document.getElementById("upfail").style.display = "none";
		...
		<div class="container">
			<div class="main_content">
				<c:if test="${empty expEdit}">
					<div class="block">
						...
						<!-- End Box Head -->
						<form:form action="#" commandName="ExperienceCmd" method="post"
							...
							<div class="block-box-small categery-topbox block-box-top-header-dept">
								...
								<div class="block-input">
											...
											<form:input path="experianceName" id="experianceDesc"  onkeyup="duplicateChecks()" class="alphaNumericValid1" 
												maxlength="30" autocomplete="off" onkeydown="removeBorder(this.id)"  tabindex="1"/>
										</div>								
								<%-- <form:errors path="experianceDesc" /> --%>
							</div>
							<div class="block-footer">
								<aside class="block-footer-left sucessfully">
									...
									<div id="upfail" style="display: block;">
										<c:forEach var="fail" items="${param.UpdateFail}">	
							             	<aside class="block-footer-left fail">
												<spring:message code="label.error" /> experience
									...
									<div class="alert-danger " id="dupMessage"
										...
										<aside class="block-footer-left fail">Warning :
											Experience already Exists. Please try Some Other</aside>
									</div>
								...
								<aside class="block-footer-right">
									<input type="button"
										class="btn-cancel"
										value="<spring:message code="label.clear" />" id="cancelId" tabindex="4"  onclick="dataClear();removeBorder(this.id)"/>
									<input type="submit" class="btn-save" id="submitId" 
										value="<spring:message code="label.save"/>" tabindex="3"/>
								</aside>
							</div>
						</form:form>
					</div>					
			...
			<div class="block">
				...
				<form:form action="" commandName="ExperienceCmd" method="post"
					...
					<div class="block-box-small categery-topbox block-box-top-header-dept">
						<form:hidden path="experianceId" />
						
						<%-- <div class="block-input">
							<label>Experience ID<span
								style="color: red;">*</span></label>
							<form:input path="experianceId" maxlength="30" autocomplete="off" class="alphaNumericValid1" onblur="validFirstLetter2(this.id)"
								onkeyup="duplicateEditCheck()"  onkeydown="removeBorder(this.id)" onfocus="removeBorder(this.id)" tabindex="2"/>
						</div> --%>
						<div class="block-input">
							<label>Experience<span
								style="color: red;">*</span></label>
							<form:input path="experianceName" maxlength="30" autocomplete="off" class="alphaNumericValid1" onblur="validFirstLetter2(this.id)"
								onkeyup="duplicateEditCheck()"  onkeydown="removeBorder(this.id)" onfocus="removeBorder(this.id)" tabindex="2"/>
						</div>
						<%-- <div class="block-input">
							<label>Experience Desc<span
								style="color: red;">*</span></label>
							<form:input path="experianceDesc" maxlength="30" autocomplete="off" class="alphaNumericValid1" onblur="validFirstLetter2(this.id)"
								onkeyup="duplicateEditCheck()"  onkeydown="removeBorder(this.id)" onfocus="removeBorder(this.id)" tabindex="2"/>
						</div> --%>
					...
					<div class="alert-danger" id="dupEditMessage"
						...
						<aside class="block-footer-right">
							<a href="ExperienceHome.htm"><input type="button" class="btn-cancel"
			...
			<div class="block table-toop-space">
				<div class="head-box">
					<h2>
						<span class="icon2">&nbsp;</span>Current Experience
					</h2>
					<form:form action="searchExp.htm" commandName="ExperienceCmd" method="get">
						<aside class="search-box">
							<input type="submit" class="search-bnt" onclick="return searchFunction()"
								value="<spring:message code="label.search"/>" tabindex="6"/>
							<form:input path="experianceName" id="addSearchId" 
				...
				<div class="block-box-dept categery-downbox block-box-last-dept">
					<ul class="table-list">
						<li class="dept-box">Experience Name
							<ul>
