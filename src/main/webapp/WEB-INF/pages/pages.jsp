						...
						function(i, ServiceListObj) {
							var tblRow = "<ul>"
								 + "<li class='dept-box'>"
									...
									+ "</li>"
									+ "<li class='eleven-box '>"
									+ '<a href="editService.htm?serviceId='
									+ ServiceListObj.pageId
									+ '" class="">Edit</a>'
									+ '</li>'
		...
		$('#serviceNameId').val('');
						...
						//AddForm Validations
								 ...
								 $('#submitId').click(function(e){
											...
											$('#addForm').attr('action',"<c:url value='/pageAdd.htm'/>");
											$("#addForm").submit();											
						...
						$('#updateId').click(function(event) {
										    ...
										    if($('#serviceName').val().length == 0 ) {
										       $('#serviceName').css('color','red');
										       $("#serviceName").css("border-color","red");
										       $("#serviceName").attr("placeholder","Please Enter Service Name");
										       $('#serviceName').addClass('your-class');
										    }										     
										    return false;										  
										    } 
										    var val=$('#serviceName').val().charCodeAt(0);
										    var val1=$('#serviceName').val();
										    var regex=new RegExp("^[a-zA-Z0-9][a-zA-Z0-9._& ]+$");
										    if(val==32 || val==95||val==38||val==46||$('#serviceName').val().length <3 )
										    	{
...
<script type="text/javascript">
	function duplicateChecks() {
		...
		$.ajax({
					...
					data : "serviceName=" + serviceName + '&categoryId='+ catId,
		...
		<div class="container">
			<div class="main_content">
				<c:if test="${empty servEdit}">
					<div class="block">
						...
						<!-- End Box Head -->
						<form:form action="" commandName="pageCmd" method="post"
							...
							<div class="block-box-small categery-topbox block-box-top-header-dept">
								...
								<div class="block-input">
											<label>Page<span
												style="color: red;">*</span></label>
											<form:input path="pageName" id="pageId" onkeyup="duplicateChecks()" class="alphaNumericValid1" 
												maxlength="30" autocomplete="off" onkeydown="removeBorder(this.id)"  tabindex="2"/>
										</div>								
			...
			<div class="block">
				...
				<form:form action="" commandName="pageCmd" method="post"
					...
					<div class="block-box-small categery-topbox block-box-top-header-dept">
						...
						<div class="block-input">
											<label><spring:message code="label.cat" /><span
												style="color: red;">*</span></label>
											<form:select path="categoryId"  cssClass="some-select" disabled="true" tabindex="1">
											<form:option value="">--Select--</form:option>
								...
								<div class="block-input">
											<label><spring:message code="label.services" /><span
												style="color: red;">*</span></label>
											<form:input path="pageName"  onkeyup="duplicateChecks()" class="alphaNumericValid1" 
												maxlength="30" autocomplete="off" onkeydown="removeBorder(this.id)"  tabindex="2"/>
										</div>
					...
					<div class="alert-danger" id="dupEditMessage"
						...
						<aside class="block-footer-right">
							<a href="serHome.htm"><input type="button" class="btn-cancel"
			...
			<div class="block table-toop-space">
				...
				<div class="block-box-dept categery-downbox block-box-last-dept">
					<ul class="table-list">
						<li class="dept-box">Service Name
							<ul>
